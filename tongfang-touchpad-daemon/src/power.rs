...
pub fn setup_touchpad_power_listener(touchpad: Arc<Mutex<Touchpad>>) -> DaemonResult<()> {
    let power = DBusProxy::for_bus_sync(
        BusType::System,
        DBusProxyFlags::NONE,
        None,
        LOGIN1_SERVICE_NAME,
    ...
    power.connect("g-signal", true, move |values| {
        let mut touchpad = touchpad.lock().unwrap();
        if let Err(e) = handle_power_signal(&mut touchpad, &state, values) {
...
fn handle_power_signal(
    ...
    let signal_name = extract_signal_name(values)?;

    if signal_name != PREPARE_FOR_SLEEP_SIGNAL {
        return Ok(());
    }

    if extract_arg(values)? {
        store_power_state(touchpad, state_store)
    } else {
        load_power_state(touchpad, state_store)
    }
}
